...
(default %Y-%m-%d %H:%M:%S).
    ...
    def _cmp(self, other, op):
        ...
        if isinstance(other, datetime):
            ...
            if op == "__eq__":
                return dt.to_tuple() == to_tuple(other)
            elif op == "__lt__":
                return dt.to_tuple() < to_tuple(other)
            elif op == "__gt__":
                return dt.to_tuple() > to_tuple(other)
            ...
            # TypeError will be automatically raised. 
...
# is uncommented, there is a small performance improvement.
...
def assert_valid_date(dt,is_leap,julian_gregorian_mixed,has_year_zero=False,
                      is_360_day=False):
